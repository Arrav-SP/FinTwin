...
def seed_database() -> dict[str, int]:
    ...
    with engine.begin() as connection:
        ...
        for i in range(1, COUNTS["transactions"] + 1):
            ...
            transaction_rows.append({"source": source if tx_type in ("TRANSFER", "WITHDRAWAL", "PAYMENT") else None, "destination": destination if tx_type in ("TRANSFER", "DEPOSIT") else None, "merchant": rng.choice(merchant_ids) if tx_type == "PAYMENT" else None, "amount": Decimal(rng.randrange(10, 5000)), "type": tx_type, "ref": f"SEED-{i:06d}", "channel": rng.choice(("BRANCH", "ATM", "ONLINE", "MOBILE", "POS"))})
